...
import anabada.semi.wrapper.EncryptWrapper;

// 필터(Filter)
// 클라이언트 요청 시 생성되는 HttpServletRequest, HttpsServletResponse
...
// destory() : 필터 코드가 변경되어 이전 필터가 필요 없어 졌을 때 수행되는 메소드

// @WebFilter : 어떤 요청을 필터링할지 주소를 작성하고
//				필터에 이름을 부여하여 필터링 순서를 지정할 수 있게하는 어노테이션

@WebFilter(filterName = "encryptFilter", urlPatterns = {"/member/login" , "/member/signUp" , "/member/myInfoModify", "/member/resignMember", "/member/updatePw"})
public class EncryptFilter implements Filter {

	public void init(FilterConfig fConfig) throws ServletException {
		System.out.println("암호와 필터 생성");
	}
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// POST의 요청일 경우 (비밀번호 입력이 포함되면 무조건 POST방식)
		if( req.getMethod().equals("POST")) {
			
			// req 요청 객체를 EncryptWrapper로 감싸기
			EncryptWrapper encWrapper = new EncryptWrapper(req);
			// -> 오버라이딩된 getParameter 사용 가능
			
			// 기존 req 대신 encWrapper 를 Servlet으로 전달
