<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>

<div><!-- ajax -->
	상점후기 <span id="postScriptText">0</span>
</div>

<hr>

<%-- <div class="userPost">
	<div class="userPostImgDiv">
		<img class="userPostImg" src="${contextPath}/resources/images/myShop/profile/캐릭터.png">
	</div>
	<div class="userPost-area">
		<div class="userPost-name">만약에나라면</div>
		<div class="userPost-star">
			<img class="userPostStarImg" src="data:image/png;base64,"
				alt="작은 별점 0점 이미지"> 
			<img class="userPostStarImg" src="data:image/png;base64,"
				alt="작은 별점 0점 이미지"> 
			<img class="userPostStarImg" src="data:image/png;base64,"
				alt="작은 별점 0점 이미지"> 
			<img class="userPostStarImg" src="data:image/png;base64,"
				alt="작은 별점 0점 이미지"> 
			<img class="userPostStarImg" src="data:image/png;base64,"
				alt="작은 별점 0점 이미지">
		</div>
		<div class="userPost-shop">이동</div>
		<div class="userPost-content">친절한 거래 감사요!</div>
		<div class="userPost-report">
			신고하기
			<button class="userPostDelete">삭제</button>
			<button class="userPostUpdate">수정</button>
		</div>
	</div>
</div>

<hr> --%>

<div class="userPostArea">
	<div class="userPostAreaImgDiv">
		<img class="userPostAreaImg" src="${contextPath}/resources/images/myShop/profile/캐릭터.png">
	</div>
	<textarea id='userPostTextarea'></textarea>
	<button id='userPostTextareaBtn'>확인<br>0/120</button>
</div>
<hr>


 <script>
 	//const postScriptNo = ${postScript.postScriptNo}
	 
 	$("#userPostTextareaBtn").on("click", function(){
 		(function(){
			$("#postScriptText").text($(".userPost").length);
		}());
 	});
 </script>




